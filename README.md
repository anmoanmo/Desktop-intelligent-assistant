...
本项目默认按公开 GitHub 仓库处理。不要提交 `.env`、`data/`、`logs/`、`config/settings.toml`、聊天记录、长期记忆、工具调用日志、运行截图、接口密钥或任何个人配置。

桌面上下文默认需要用户确认后才采集；主动提醒和自动记忆提取默认关闭。开启这些能力前，请确认你接受将窗口标题、焦点文本、长期记忆等上下文发送给配置的大模型服务商。

如果密钥、令牌、私钥或个人资料曾经进入远端仓库，不要只删除文件；应先吊销/轮换对应凭据，再清理 Git 历史并强制推送。
